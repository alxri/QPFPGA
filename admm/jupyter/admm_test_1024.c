...
// Directory where Python dumped the 1024x1024 binary files
#define DATA_DIR "/home/xilinx/admm/baremetal/data_1024/"
...
void load_float_bin(const char* filename, float* dest, size_t count) {
    ...
    fread(dest, sizeof(float), count, f);
    fclose(f);
}
...
void load_int_bin(const char* filename, int* dest, size_t count) {
    FILE* f = fopen(filename, "rb");
    if (!f) { printf("ERROR: Could not open %s\n", filename); exit(1); }
    fread(dest, sizeof(int), count, f);
    fclose(f);
}

void pack_csc_to_words(int nnz, const int *row_idx, const float *values, int32_words *row_words, float32_words *val_words) {
...
void transpose_csc(int rows, int cols, const int *col_ptr, const int *row_idx, const float *values, int *col_ptr_t, int *row_idx_t, float *values_t) {
...
// =====================================================================
int main() {
    printf("\n--- Starting ADMM Hardware Testbench (1024x1024 File Loaded) ---\n");
    ...
    printf("Loading Problem Data from %s...\n", DATA_DIR);
    char filepath[256];

    sprintf(filepath, "%s%s", DATA_DIR, "x_true.bin"); load_float_bin(filepath, x_true, NUM_COLS);
    sprintf(filepath, "%s%s", DATA_DIR, "q.bin"); load_float_bin(filepath, q, NUM_COLS);
    sprintf(filepath, "%s%s", DATA_DIR, "P_values.bin"); load_float_bin(filepath, P_values_cpu, NUM_COLS);
    sprintf(filepath, "%s%s", DATA_DIR, "P_col_ptr.bin"); load_int_bin(filepath, P_col_ptr_cpu, NUM_COLS + 1);
    ...
    sprintf(filepath, "%s%s", DATA_DIR, "A_values.bin"); load_float_bin(filepath, A_values_cpu, A_nnz);
    sprintf(filepath, "%s%s", DATA_DIR, "A_col_ptr.bin"); load_int_bin(filepath, A_col_ptr_cpu, NUM_COLS + 1);
    sprintf(filepath, "%s%s", DATA_DIR, "A_row_idx.bin"); load_int_bin(filepath, A_row_idx_cpu, A_nnz);
    sprintf(filepath, "%s%s", DATA_DIR, "l_in.bin"); load_float_bin(filepath, l_cpu, NUM_ROWS);
    sprintf(filepath, "%s%s", DATA_DIR, "u_in.bin"); load_float_bin(filepath, u_cpu, NUM_ROWS);
    sprintf(filepath, "%s%s", DATA_DIR, "rho_in.bin"); load_float_bin(filepath, rho_cpu, NUM_ROWS);
    ...
    transpose_csc(NUM_ROWS, NUM_COLS, A_col_ptr_cpu, A_row_idx_cpu, A_values_cpu, AT_col_ptr_cpu, AT_row_idx_cpu, AT_values_cpu);
    ...
    printf("--- Output Preview (First 10) ---\n");
    for (int i = 0; i < 10; i++) {
    ...
    free(x_true); free(q); free(A_col_ptr_cpu); free(A_row_idx_cpu); free(A_values_cpu);
